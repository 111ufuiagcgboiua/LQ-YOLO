...
class ELAN_E_PPCB(nn.Module):
    ...
    def __init__(self, c1, c2, in_c, k=1, s=1, p=None, g=1, layer=[],
        ...
        if backbone:
            ...
            self.cv1 = Conv(c1, c_, k=k, s=s, p=p, g=g, act=act)
            self.cv2 = PPCB(c1, c_)
            self.cv4 = DWConv(c_, c_, k=3, s=s, act=act)
            self.cv6 = Conv(c_, c_, k=3, s=s, p=p, g=g, act=act)
        else:
            c_ = in_c
            c_out = c_ * 4 + c2 * 2
...
class ELAN_E_DWConv(nn.Module):
    ...
    def __init__(self, c1, c2, in_c, k=1, s=1, p=None, g=1, layer=[],
        ...
        if backbone:
            ...
            self.cv1 = Conv(c1, c_, k=k, s=s, p=p, g=g, act=act)
            self.cv2 = Conv(c1, c_, k=k, s=s, p=p, g=g, act=act)
            self.cv3 = DWConv(c_, c_, k=3, s=s, act=act)
        else:
            c_ = in_c
            c_out = c_ * 4 + c2 * 2
            self.cv1 = Conv(c1, c2, k=k, s=s, p=p, g=g, act=act)
        ...
        self.cv4 = Conv(c_, c_, k=3, s=s, p=p, g=g, act=act)
        self.cv5 = DWConv(c_, c_, k=3, s=s, act=act)
        self.cv6 = Conv(c_, c_, k=3, s=s, p=p, g=g, act=act)
        self.cv7 = Conv(c_out, c2, k=k, s=s, p=p, g=g, act=act)
...
class DCNV3_YoLo(nn.Module):
    def __init__(self, inc, ouc, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        self.dcnv3 = DCNv3_pytorch(ouc, kernel_size=k, stride=s, group=g, dilation=d)
...
class DCARF(nn.Module):
    def __init__(self, inc):
        super(DCARF, self).__init__()

        self.adjust_conv = nn.Identity()
        if inc[0] != inc[1]:
            self.adjust_conv = nn.Conv2d(inc[0], inc[1], kernel_size=1)

        # 假设 DCNV_Yolo 模块已经定义好
        self.dcnv_yolo_x0 = DCNV3_YoLo(inc[0], inc[0], k=3, s=1, p=1)
        self.dcnv_yolo_x1 = DCNV3_YoLo(inc[1], inc[1], k=3, s=1, p=1)
        #self.dWconv = DWConv(inc[0], inc[1], k=3, s=1)

        self.se = SEAttention(inc[1] * 2)  # 注意：这通常不会直接应用于 x0 和 x1
        # 假设融合后的通道数与较大的输入通道数相同

    def forward(self, x):
        x0, x1, *rest = x
        x0 = self.adjust_conv(x0)

        x0 = self.dcnv_yolo_x0(x0)
        x1 = self.dcnv_yolo_x1(x1)
        ...
        x_weight = torch.cat([x0 + x1_weight, x1 + x0_weight], dim=1)
        return x_weight
